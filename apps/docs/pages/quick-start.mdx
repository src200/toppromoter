...
Stripe will automatically send a webhook to Toppromoter when a new payment is received. From there, we will check to see if the `reflio_referral_id` exists on the customer object.
...
```
If the current visitor came to your site from an affiliate link, Toppromoter will detect the `data-toppromoter` attribute on each form and automatically insert a hidden input named `reflio_referral_id` with the unique referral ID as the value. For example:
``` html copy
